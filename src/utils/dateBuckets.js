...
export function getBucket(eventDate, now = new Date()) {
  ...
  if (eventTime < todayStart) {
    const recentPastCutoff = new Date(now);
    recentPastCutoff.setDate(recentPastCutoff.getDate() - DAYS_FOR_RECENT_PAST);
    recentPastCutoff.setHours(0, 0, 0, 0);

    if (eventTime >= recentPastCutoff.getTime()) {
      return "LAST_30_DAYS";
    }
